...
function PaperCard({ paper }) {
  ...
  return (
    <div
      className={`border-2 rounded-xl p-5 transition-all hover:border-blue-200 ${paper.isSample ? "border-amber-200 bg-amber-50/30" : "border-gray-100"}`}
    >
      {" "}
      <div className="flex items-start justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {" "}
          {paper.isSample && <Badge variant="sample">Sample</Badge>}{" "}
      ...
      {paper.file_name ? (
        <div className="flex items-center gap-2 text-xs text-green-600 bg-green-50 px-2 py-1 rounded mb-2">
          ...
          {paper.file_size && (
            <span className="text-gray-400">({paper.file_size})</span>
          )}{" "}
        </div>
      ) : (
        <div className="flex items-center gap-2 text-xs text-orange-600 bg-orange-50 px-2 py-1 rounded mb-2">
          {" "}
          <AlertTriangle size={12} /> <span>No file attached</span>{" "}
        </div>
      )}{" "}
      <div className="flex items-center justify-between">
        {" "}
        <p className="text-xs text-gray-400">
          Uploaded: {paper.upload_date}
        </p>{" "}
        {paper.isSample ? (
