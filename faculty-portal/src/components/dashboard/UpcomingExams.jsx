...
function UpcomingExams({ exams }) {
  ...
  return (
    <Card>
      ...
      <Card.Header>
        {" "}
        <Card.Title icon={CalendarClock}>Upcoming Exams</Card.Title>{" "}
      </Card.Header>{" "}
      <Card.Content>
        {" "}
        <div className="space-y-3">
          ...
          {upcomingExams.map((exam, index) => (
            <div
              key={index}
              className={`p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors ${exam.isSample ? "border-l-4 border-amber-400" : ""}`}
            >
              {" "}
              <div className="flex items-start justify-between">
                {" "}
                <p className="font-medium text-sm">{exam.exam_name}</p>{" "}
                {exam.isSample && <Badge variant="sample">Sample</Badge>}{" "}
              </div>{" "}
              ...
              <div className="flex items-center gap-2 mt-2 text-xs text-gray-600">
                ...
                <Clock size={12} className="ml-2" /> {exam.exam_time}{" "}
              </div>{" "}
