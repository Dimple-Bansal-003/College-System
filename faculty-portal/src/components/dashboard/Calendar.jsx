      ...
      <Card.Content>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {calendarDays.map((dayData, index) => {
            const dayExams = dayData.dateStr
              ? getExamsOnDate(dayData.dateStr)
              : [];
            const hasExams = dayExams.length > 0;
            return (
              <div
                key={index}
                onClick={() => hasExams && setSelectedDate(dayData.dateStr)}
                className={`aspect-square flex flex-col items-center justify-start p-1 rounded-lg text-sm transition-all cursor-pointer ${dayData.day === null ? "bg-gray-50" : ""} ${dayData.isToday ? "bg-blue-100 ring-2 ring-blue-500 font-bold" : ""} ${hasExams ? "bg-amber-100 hover:bg-amber-200" : "hover:bg-gray-100"} `}
              >
                {" "}
                {dayData.day && (
                  <>
                    {" "}
                    <span className={dayData.isToday ? "text-blue-600" : ""}>
                      {dayData.day}
                    </span>{" "}
                    {hasExams && (
        ...
        <div className="flex items-center gap-6 mt-4 pt-4 border-t border-gray-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">Has Exam</span>{" "}
          </div>{" "}
        </div>{" "}
      </Card.Content>{" "}
    </Card>
